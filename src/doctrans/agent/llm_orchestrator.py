...
document. Disabled by default; validates its own output and on any problem the caller
keeps the rule result. Default deployment makes zero paid API calls and is fully
deterministic. **Never rewrites a translation, changes structure, or alters placeholders.**
"""
...
class LLMBrain:
    ...
    def consistency_note(self, src_lang: str, tgt_lang: str, sample: str) -> Optional[str]:
        """A one-line terminology/consistency note over a sample. None to keep the rule rationale."""
        ...
        prompt = (f"You are a localization reviewer. In ONE short sentence, flag any terminology "
                  f"inconsistency or obvious error in this {src_lang}->{tgt_lang} document sample. "
