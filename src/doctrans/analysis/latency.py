"""Latency benchmark: end-to-end document-agent throughput (offline).

Builds a single ``DocTransAgent`` (line/regex parsers + dictionary MT) and times
``.run(doc)`` over the seed documents. Reports p50/p95/p99/mean per-doc latency_ms,
throughput, decision-presence (D1-D5), and the status mix.
"""
...
def benchmark(cfg: AppConfig = None, n: int = 10, warmup: int = 2, save: bool = True) -> Dict:
    ...
    try:
        ...
        docs = [d["src"] for d in samples.docs()] or ["# Title\n\nA paragraph."]
    ...
    total = n + warmup
    workload = (docs * (total // len(docs) + 1))[:total]
    for d in workload[:warmup]:
        try:
            agent.run(d, save=False)
        except Exception:
            pass
