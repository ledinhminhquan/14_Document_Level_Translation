...
Reuses the P13 ``s2st`` translator design:
* ``DictionaryTranslator`` — a word-lookup baseline (no torch): the offline floor + the
  fallback that lets the whole pipeline run with no GPU/network.
* ``TransformerTranslator`` — wraps a fine-tuned/pretrained seq2seq model
  ...
  source/target language-code conventions, and supporting a ``<BRK>`` context separator.

Both expose ``translate(text)`` and ``translate_batch(texts)`` plus ``name`` / ``version``.
...
# match masking sentinels (e.g. [[PH0]]) as a single token so the dictionary baseline
# never splits them apart (a real MT model keeps them intact too).
_WORD = re.compile(r"\[\[PH\d+\]\]|[A-Za-zÀ-ÿ']+|\d+|[^\w\s]", re.IGNORECASE)
_SENT = re.compile(r"\[\[PH\d+\]\]")
...
class DictionaryTranslator:
    ...
    def translate(self, text: str) -> str:
        ...
        for tok in _WORD.findall(text or ""):
            if _SENT.fullmatch(tok):           # a masking sentinel: pass through verbatim
                out.append(tok)
                continue
...
class TransformerTranslator:
    ...
    def _detect_family(self) -> str:
        ...
        if "m2m100" in cls:
            return "m2m100"
        if hasattr(self.tok, "get_lang_id"):
            return "m2m100"
