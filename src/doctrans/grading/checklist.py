...
_SUBPACKAGES = ["data", "models", "docparse", "mt", "training", "agent", "api",
...
_REQUIRED_SRC = [
    ...
    f"src/{PKG}/docparse/segment.py", f"src/{PKG}/docparse/mask.py", f"src/{PKG}/docparse/router.py",
    f"src/{PKG}/docparse/markdown_doc.py", f"src/{PKG}/docparse/structure_score.py",
    f"src/{PKG}/mt/translator.py", f"src/{PKG}/mt/context.py",
    f"src/{PKG}/training/train_mt.py", f"src/{PKG}/training/evaluate.py", f"src/{PKG}/training/metrics.py",
    f"src/{PKG}/agent/doctrans_agent.py", f"src/{PKG}/agent/policy.py", f"src/{PKG}/api/main.py",
]
...
_REQUIRED_DOCS = [
    ...
    "ethics_statement", "architecture", "evaluation", "model_card", "slide_deck_outline", "DESIGN_BRIEF",
]
...
_REQUIREMENTS = {
    ...
    "R2_model_baseline": ("Model selection + baseline (dictionary/zero-shot) vs MT core",
                          ["docs/model_selection.md", f"src/{PKG}/mt/translator.py",
                           f"src/{PKG}/training/train_baseline.py"]),
    "R3_training_eval": ("Training + evaluation with metrics (chrF/BLEU + SPS)",
                         [f"src/{PKG}/training/train_mt.py", f"src/{PKG}/training/evaluate.py", "docs/evaluation.md"]),
    "R4_agent": ("Agentic document pipeline with explicit decision points",
                 [f"src/{PKG}/agent/doctrans_agent.py", f"src/{PKG}/agent/policy.py", "docs/agent_architecture.md"]),
...
def build_checklist(repo) -> Dict:
    ...
    check("Functional: evaluate() produces chrF + document SPS", eval_res["ok"],
...
def _check_import() -> Dict:
    try:
        import importlib
        for m in (PKG, f"{PKG}.config", f"{PKG}.agent.doctrans_agent", f"{PKG}.training.evaluate"):
...
def _check_agent_offline() -> Dict:
    try:
        ...
        job = agent.run(samples.docs()[0]["src"], fmt="markdown", save=False)
        data = job.to_dict()
...
def _check_evaluate() -> Dict:
    try:
        ...
        ok = "chrf" in m and ("doc_sps" in doc or "doc_chrf" in doc)
        return {"ok": bool(ok), "detail": f"model chrF={m.get('chrf')} doc_sps={doc.get('doc_sps')}"}
    except Exception as exc:  # pragma: no cover
